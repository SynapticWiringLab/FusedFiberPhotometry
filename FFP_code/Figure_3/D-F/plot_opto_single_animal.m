...
% import data and calculate autofluorescence of the fiber
...
subplot(3, 3, 1:3);
plot([1:length(LaserStim)]./d.sampling_rate, rescale(LaserStim,min([d.analog_1_s, d.analog_2_s]),max([d.analog_1_s, d.analog_2_s])), 'color', [255, 153, 51]./255); hold on; 
plot([1:length(d.analog_2_s)]./d.sampling_rate, d.analog_2_s, 'color', [115, 0, 230]./255);
plot([1:length(d.analog_1_s)]./d.sampling_rate, d.analog_1_s, 'color', [51, 133, 255]./255);
xlim([0 length(d.analog_1_s)./d.sampling_rate]);
...
plot_areaerrorbar(d.d470_rel_Stim_norm, c); % create actual plot
...
plot_areaerrorbar(d.d405_rel_Stim_norm, c); % create actual plot
xlim([-1 twindow]); % align time axix
linkaxes([ax1, ax2])
...
plot_areaerrorbar(d.dFoF_rel_Stim_norm, c); % create actual plot
xlim([-1 twindow]); % align time axix
linkaxes([ax1, ax2, ax3]); ylim([-0.2 0.6])
